...
func TestBuildRetrySqls(t *testing.T) {
	// before=3600 duration=600 => st=11:00:00 ed=11:10:00
	...
	tests := []struct {
	...
	}{
		{
			...
			wantUpdateSql: "UPDATE `t_order` SET status = 1 WHERE `status_at` >= " + stStr + " AND `status_at` < " + edStr + " AND status = 0",
		},
		{
			name:          "timestamp",
			columnType:    ColumType_Timestamp,
			...
			wantUpdateSql: "UPDATE `t_order` SET status = 1 WHERE `status_at` >= '2026-09-21 11:00:00' AND `status_at` < '2026-09-21 11:10:00' AND status = 0",
		},
		{
			name:          "datetime",
			columnType:    ColumType_Datetime,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			countSql, updateSql, err := BuildRetrySqls("t_order", "status_at", tt.columnType,
				"status = 0", "status = 1", 3600, 600, baseNow)
			if err != nil {
				t.Fatalf("unexpected err: %s", err)
			}
...
func TestBuildRetrySqlsInvalidColumnType(t *testing.T) {
	_, _, err := BuildRetrySqls("t_order", "status_at", "bad_type", "status = 0", "status = 1", 3600, 600, baseNow)
	if err == nil {
		t.Fatal("expected error for invalid column_type, got nil")
	}
}
...
func TestRetryParseSqlMatchesBuilder(t *testing.T) {
	...
	_, want, err := BuildRetrySqls(cfg.Tablename, cfg.ColumnName, cfg.ColumnType,
		cfg.FindWh, cfg.SetFields, cfg.Before, cfg.Duration, time.Now())
	if err != nil {
		t.Fatalf("unexpected err: %s", err)
	}
